...
module.exports = (robot) ->
  ...
  robot.respond /reload users/i, (res) ->
    robot.http("https://slack.com/api/users.list?token=" + process.env.HUBOT_SLACK_TOKEN).get() (err, res, body) ->
      data = JSON.parse(body)
      
      for member in data.members
        robot.brain.set(member.name, "in office")

      console.log(robot.brain.get("wes"))

  robot.respond /I(\'m| am) (ooo|out of (the )?office)/i, (res) ->
    robot.brain.set("#{res.message.user.name.toLowerCase()}", "out of office")
    console.log(robot.brain.get("#{res.message.user.name.toLowerCase()}"))
